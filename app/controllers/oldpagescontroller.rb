class PagesController < ApplicationController
  ...
  def index
    ...
    if params[:nb_jour_pre].present? && params[:nb_jour_post].present?
      ...
      @results_sql = calquery(current_user.id, 5)
    end
    usereventbuilder
    set_insta
          puts '-----------------'
  ...
  def calquery(id, jour)
    sql = "with alldate as
((select 'memo' as categorie, m.id, m.calendardate as date, extract(year from m.calendardate) as year,extract(month from m.calendardate) as month,extract(day from m.calendardate) as day, m.content as description from memos m
where m.user_id = #{id} and calendardate is not null)
union
(select 'keyddate' as categorie, k.id, k.date + (extract(year from current_date) - EXTRACT(YEAR FROM k.date) || ' years')::interval as date, extract(year from current_date) as year, extract(month from k.date) as month,extract(day from k.date) as day, k.description as description from key_dates k
where k.user_id = #{id})
union
(select 'event' as categorie, e.id, e.start_date as date, extract(year from e.start_date) as year, extract(month from e.start_date) as month , extract(day from e.start_date) as day ,  case when ((e.title=' ') or (e.title is null)) then LEFT(e.description, 30)
